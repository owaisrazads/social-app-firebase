...
const loggedInUserId = 'user123'; // Replace this with your actual user ID

// Fetch todo list for the logged-in user
db.collection('todos')
  .where('userId', '==', loggedInUserId)
  .get()
  .then((querySnapshot) => {
    querySnapshot.forEach((doc) => {
      // Access each todo item here
      const todoData = doc.data();
      console.log('Todo: ', todoData);
      // Perform actions with each todo item
    });
  })
  .catch((error) => {
